...
export async function signIn(username, password, context) {
  ...
  export async function signUpVehicle(signUpVehicle, context) {
      try {
        const response = await axios.post(SPRING_BACKEND_URL+'auth/api/v1/register/vehicle', signUpVehicle);
        saveSession(response.data.token, response.data.wrapped.plate, context)
        return response
      } catch (error) {
        throw new Error(error);
      }
